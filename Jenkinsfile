...
pipeline {
    ...
    stages {
        ...
        stage('Build and Run Docker') {
            steps {
                sh "docker build -t my-roundhouse-image ."
                sh "docker run --rm -e TARGET=$TARGET -e ENVIRONMENT=$ENVIRONMENT -e DATABASENAME=$DATABASE_NAME -e SERVERNAME=$SERVER_NAME -e USERID=$USERID -e PASSWORD=$PASSWORD my-roundhouse-image:latest"
